...
application (Claude Desktop, Cursor, Cline, Continue, etc.) read your GymTrack profile —
...
session secrets — it can only read the same `state-<uid>.json` files the GymTrack api already
writes.
...
GYMTRACK_UID=<your-uid> GYMTRACK_DATA=/path/to/GymTrack/data node src/index.js
```
...
```jsonc
{
  "mcpServers": {
    "gymtrack": {
      "command": "node",
      "args": ["/absolute/path/to/GymTrack/mcp/src/index.js"],
      "env": {
        "GYMTRACK_DATA": "/absolute/path/to/GymTrack/data",
        "GYMTRACK_UID": "<your-uid>"   // optional — auto-detected if you have one profile
      }
    }
  }
}
...
Restart the client; you should see the GymTrack tools appear with "serving profile \<name\>" on
the server's stderr.
...
AGPL-3.0-or-later, same as GymTrack.
