#!/usr/bin/env node
/* GymTrack MCP server — stdio transport. The LLM client (Claude Desktop, Cursor, …) spawns
...
// Fail fast on bad config so a misnamed GYMTRACK_DATA doesn't silently answer every call with
...
try {
  init()
  const u = getUser()
  console.error(`[gymtrack-mcp] serving profile ${u.name} (${u.id})`)
} catch (e) {
  console.error(`[gymtrack-mcp] ${e.message}`)
