/* gymtrack-mcp state — reads ./data/state-<uid>.json + db.json (read-only). Cached with an
...
const DATA_DIR = process.env.GYMTRACK_DATA || path.join(process.cwd(), 'data')
...
// Pick the uid: GYMTRACK_UID env, else the only state-* file, else the only user in db.json.
// Throws listing the options if ambiguous. The sanitiser on stateFile() keeps a sneaky
// '..' in GYMTRACK_UID harmless.
function resolveUid() {
  const envUid = (process.env.GYMTRACK_UID || '').trim()
  if (envUid) {
    if (!/^[a-zA-Z0-9_-]+$/.test(envUid)) throw new Error(`GYMTRACK_UID contains characters that aren't safe in a filename: ${JSON.stringify(envUid)}`)
    return envUid
  }
  ...
  if (files.length === 0) {
    ...
    if (_db.users.length === 0) throw new Error(`no GymTrack users found in ${path.join(DATA_DIR, 'db.json')} — sign in at least once on a device`)
    ...
    throw new Error(
      `multiple GymTrack users found — set GYMTRACK_UID to one of: ${_db.users.map(u => u.id).join(', ')}\n` +
      `  (look them up in ${path.join(DATA_DIR, 'db.json')} under "users"[].id)`
    )
  }
  throw new Error(
    `multiple GymTrack users found — set GYMTRACK_UID to one of: ${files.join(', ')}\n` +
    `  (look them up in ${path.join(DATA_DIR, 'db.json')} under "users"[].id)`
  )
}
...
export function init() {
  if (_uid !== null) return
  if (!fs.existsSync(DATA_DIR)) throw new Error(`GYMTRACK_DATA dir does not exist: ${DATA_DIR}`)
  _uid = resolveUid()
  reloadDb()
