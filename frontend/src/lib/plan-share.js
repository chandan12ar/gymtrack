...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR GymTrack — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { gymtrack_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.gymtrack_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an GymTrack plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with GymTrack'))}</footer>
</div></body></html>`
}
