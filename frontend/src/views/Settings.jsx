...
export default function Settings() {
  ...
  const doExport = async () => {
    ...
    const name = 'gymtrack-backup-' + todayISO() + '.json'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not an GymTrack backup')
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host GymTrack')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install GymTrack as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.6 }}>
      GymTrack · {t('free & open source (AGPL v3)')}<br />
      <a href="https://github.com/chandan12ar/gymtrack" target="_blank" rel="noopener">source code</a> · exercise data: hasaneyldrm/exercises-dataset (CC)
    </div>
  </div>
}
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if GymTrack is closed.')}>
