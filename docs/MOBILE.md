...
GymTrack ships in two flavors from the same codebase:
...
State is mirrored from `localStorage` into `gymtrack-state.json` in the app's private data
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which GymTrack doesn't do.
...
GymTrack's mobile app isn't built or published yet — the codebase supports it (same Capacitor
setup as upstream openGym), but the Android bundle ID (`ch.duartesantos.opengym`) still needs
renaming before a build is worth signing and distributing. Deferred until actually needed.
...
keytool -genkeypair -keystore my.keystore -alias gymtrack -keyalg RSA -validity 10950

# align + sign (zipalign/apksigner ship with the Android SDK build-tools)
zipalign -f -p 4 app-release-unsigned.apk aligned.apk
apksigner sign --ks my.keystore --ks-key-alias gymtrack --out GymTrack.apk aligned.apk
```
...
- **License:** GymTrack is AGPL-3.0, which by itself sits badly with app-store terms of
